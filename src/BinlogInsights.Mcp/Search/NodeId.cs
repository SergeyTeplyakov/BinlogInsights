...
/// <summary>
/// Round-trippable identifier for any <see cref="BaseNode"/> in a loaded build.
/// <para>
/// For a <see cref="TimedNode"/> the id is its <see cref="TimedNode.Index"/>, e.g. <c>"42"</c>.
/// For any other node the id is <c>"&lt;nearestTimedNodeAncestorIndex&gt;/&lt;ord&gt;.&lt;ord&gt;..."</c>,
/// where each ordinal is the child index walking down from that ancestor.
/// E.g. <c>"42/3.7"</c> = child 7 of child 3 of the node with <c>Index == 42</c>.
/// </para>
/// <para>
/// <b>Scope:</b> ids are derived from deserialization order, so they are stable for the same
/// binlog file bytes but are NOT portable across different binlog files. Once a binlog is
/// overwritten by a new build, discard previously returned ids.
/// </para>
/// </summary>
internal static class NodeId
{
    ...
    /// <summary>
    /// Resolves a node id (as produced by <see cref="Get"/>) back to a <see cref="BaseNode"/>.
    /// Throws if the id cannot be parsed or the referenced node does not exist.
    /// </summary>
    public static BaseNode Resolve(TimedNode[] indexMap, string id)
    {
        ...
        if (!int.TryParse(indexPart, NumberStyles.Integer, CultureInfo.InvariantCulture, out int index))
        {
            throw new ArgumentException(
                $"Invalid node id: '{id}'. Expected an integer or '<int>/<ord>.<ord>...'.", nameof(id));
        }

        if ((uint)index >= (uint)indexMap.Length || indexMap[index] is not TimedNode anchor)
        {
        ...
        foreach (var ordinalText in tail.Split('.'))
        {
            ...
            {
                throw new ArgumentException(
                    $"Invalid node id: '{id}'. Ordinal '{ordinalText}' is not an integer.", nameof(id));
            }

            if (current is not TreeNode parent || ordinal < 0 || ordinal >= parent.Children.Count)
            {
                throw new KeyNotFoundException(
                    $"Node id '{id}' does not resolve: ordinal {ordinal} is out of range.");
            }
